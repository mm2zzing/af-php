...
use af\tests\kernel\actor\Test_Component;
...
class Actor_Test extends Test_Case {
  ...
  public function test_add_child() {
    $child = $this->kernel->new_object('af\kernel\actor\Actor', '/test1');
  ...
  public function test_add_component() {
    $find_component = $this->actor->get_component('af\kernel\actor\Component');
    $this->assert(true == $find_component->is_null(), 'find component is null');
    $component = $this->actor->add_component('af\kernel\actor\Component');
    $find_component = $this->actor->get_component('af\kernel\actor\Component');
    $this->assert(false == $find_component->is_null(), 'find component is not null');
  }

  public function test_add_components() {
    $find_component = $this->actor->add_component('af\tests\kernel\actor\Test_Component');
    $find_component = $this->actor->add_component('af\tests\kernel\actor\Test_Component');
    $find_component = $this->actor->add_component('af\kernel\actor\Component');
    $components = $this->actor->get_components('af\tests\kernel\actor\Test_Component');
    $this->assert(2 == count($components), 'Test_Component count is 2');
    $components = $this->actor->get_components('af\kernel\actor\Component');
    $this->assert(1 == count($components), 'Component count is 1');
  }

  public function test_remove_component() {
    $this->actor->add_component('af\kernel\actor\Component');
    $this->actor->remove_component('af\kernel\actor\Component');
    $component = $this->actor->get_component('af\kernel\actor\Component');
    $this->assert(true == $component->is_null(), 'component is null');
  }

  public function set_up() {
    $this->kernel = new Kernel();
    $this->actor = $this->kernel->new_object('af\kernel\actor\Actor', '/temp/actor1');
  }
