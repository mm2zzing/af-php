...
use af\plugins\auth\models\auth;
use af\plugins\auth\accessors\User;
use af\plugins\auth\dto\User_DTO;
use af\tests\plugins\auth\Mock_Auth_Storage;
...
class Auth_Test extends Test_Case {
  ...
  public function set_up() {
    $actor = Kernel::get_instance()->new_object('af\kernel\actor\Actor', '/sys/auth');
    $this->auth = $actor->add_component('af\plugins\auth\models\Auth');
    $storage = $actor->add_component('af\tests\plugins\auth\Mock_Auth_Storage');
    $this->auth->start($storage, 'af\plugins\auth\dto\User_DTO');
  }
